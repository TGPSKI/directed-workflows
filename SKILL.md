...
description: "Creates directed workflow files for any repository. Generates structured markdown workflows that AI agents execute interactively to guide users through multi-step configuration processes like service onboarding, environment setup, and infrastructure provisioning. Use when asked to create a directed workflow, encode a process, or build an agent-guided configuration flow."
---
...
Inspect the user's workspace to understand:
...
Write the files into the user's workspace at `.agents/commands/`. Study the examples in the skill directory for structural reference:
...
Use `templates/single-file-workflow.md` or `templates/multi-phase-router/` as your starting skeleton. These are in the skill directory, not the user's workspace.
